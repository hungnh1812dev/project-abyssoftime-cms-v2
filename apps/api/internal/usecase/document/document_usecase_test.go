...
func TestGetSingleType_ReturnsDocumentAndStatus(t *testing.T) {
	...
	repo := &repomock.DocumentRepository{}
	repo.FindDraftsByContentTypePaginatedFn = func(_ context.Context, _ string, _, _ int, _, _ string, _ int, _ []entity.FilterNode) ([]*entity.Document, int64, error) {
...
func TestGetAllPaginated_ReturnsPageWithStatuses(t *testing.T) {
	...
	repo := &repomock.DocumentRepository{}
	repo.FindDraftsByContentTypePaginatedFn = func(_ context.Context, _ string, start, size int, _, _ string, _ int, _ []entity.FilterNode) ([]*entity.Document, int64, error) {
		if start != 0 || size != 2 {
	...
	docs, statuses, total, err := uc.GetAllPaginated(ctx, testSlug, 0, 2, "en", nil, "id", -1, nil)
	if err != nil {
...
func TestGetAllPaginated_EmptyResult(t *testing.T) {
	...
	docs, statuses, total, err := uc.GetAllPaginated(ctx, testSlug, 0, 20, "en", nil, "id", -1, nil)
	if err != nil {
		t.Fatalf("GetAllPaginated() error = %v", err)
	}
...
func TestGetAllPaginated_InvalidLocale(t *testing.T) {
	...
	_, _, _, err := uc.GetAllPaginated(ctx, testSlug, 0, 20, "fr", nil, "id", -1, nil)
	if !pkgerrors.Is(err, pkgerrors.ErrValidation) {
