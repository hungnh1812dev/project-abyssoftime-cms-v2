...
describe('Dark mode token completeness', () => {
  const newTokens = ['primary-hover', 'primary-active', 'primary-muted', 'success', 'success-foreground', 'warning', 'warning-foreground', 'sidebar-muted'];
  ...
  it('@theme inline registers all new color tokens', () => {
    ...
    for (const token of newTokens) {
      expect(themeBlock, `Missing @theme registration: --color-${token}`).toContain(`--color-${token}`);
    }
  });
});
