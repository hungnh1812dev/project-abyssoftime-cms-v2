...
describe('ContentTypeLayout', () => {
  ...
  it('renders status badge when status is provided', () => {
    render(
      <ContentTypeLayout title="Post" status="draft">
        <span />
      </ContentTypeLayout>,
    );
  ...
  it('renderHeader replaces the entire header row', () => {
    render(
      <ContentTypeLayout title="Blog" status="published" renderHeader={() => <div data-testid="custom-header">Custom</div>}>
        <span />
      </ContentTypeLayout>,
    );
