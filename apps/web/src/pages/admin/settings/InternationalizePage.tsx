...
export function InternationalizePage() {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (editingLocale) {
      updateLocale.mutate(
        { code: editingLocale.code, name: formName, isDefault: formIsDefault },
        { onSuccess: () => setDialogOpen(false) },
      )
    } else {
      createLocale.mutate(
        { code: formCode, name: formName, isDefault: formIsDefault },
        { onSuccess: () => setDialogOpen(false) },
      )
    }
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Lowercase, 2-5 characters (e.g., en, vi, zh-cn)
              </p>
            </div>
            <div className="space-y-2">
          ...
          {locales.map((locale) => (
            <TableRow key={locale.code}>
              ...
              <TableCell className="text-center">
                {locale.isDefault && <Star className="inline h-4 w-4 text-amber-500 fill-amber-500" />}
              </TableCell>
