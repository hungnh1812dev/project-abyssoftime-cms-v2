...
beforeEach(() => {
  ...
  mock.onGet('/api/document-manager/blog-posts/doc-1').reply(200, doc)
})
...
describe('CollectionDetailPanel', () => {
  ...
  it('shows success toast and resets form to clean after successful save', async () => {
    ...
    const user = userEvent.setup()
    mock.onPut('/api/document-manager/blog-posts/doc-1').reply(200, { ...doc, data: { title: 'Updated', body: 'Some content' } })
  ...
  it('shows error toast and preserves values on failed save', async () => {
    const { toast } = await import('sonner')
    const user = userEvent.setup()
    mock.onPut('/api/document-manager/blog-posts/doc-1').reply(422, { error: 'Validation failed' })
  ...
  it('shows Unpublish button when status is published', async () => {
    mock.onGet('/api/document-manager/blog-posts/doc-1').reply(200, { ...doc, status: 'published' })
    renderWithProviders(<CollectionDetailPanel contentType={ct} documentId="doc-1" />)
    await waitFor(() =>
  ...
  it('calls POST /publish when Publish button is clicked', async () => {
    const user = userEvent.setup()
    mock.onPost('/api/document-manager/blog-posts/doc-1/publish').reply(200, { status: 'published' })
    renderWithProviders(<CollectionDetailPanel contentType={ct} documentId="doc-1" />)
  ...
  it('shows both Publish and Unpublish buttons when status is modified', async () => {
    mock.onGet('/api/document-manager/blog-posts/doc-1').reply(200, { ...doc, status: 'modified' })
    renderWithProviders(<CollectionDetailPanel contentType={ct} documentId="doc-1" />)
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /^publish$/i })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: /unpublish/i })).toBeInTheDocument()
